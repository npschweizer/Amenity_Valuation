...
df_l1 %>%
  ggplot(aes(x=count.x, y=rental_income/bedrooms))+
  geom_smooth() + 
  geom_point()
...
df_l1 %>%
  ggplot(aes(x=count.x, y=rental_income))+
  geom_smooth() + 
  geom_point()

plot(lm(rental_income ~count.x + reviews_count , data=df_l1)) 

df_cort=data.frame(df_l1%>% select_if(is.numeric)%>%cor(use="pairwise.complete.obs"))
df_cort$count.x
```
